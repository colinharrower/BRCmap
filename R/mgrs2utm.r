#' Convert MGRS grid reference to UTM grid reference
#'
#' @description A function to take one or more grid references using the Military Grid Reference System (MGRS) and to convert them to grid references using the Universal Transverse Mercator (UTM) system. 
#'
#' @param mgrs Character vector containing MGRS grid reference(s) to be converted to UTM grid references
#' @param centre Logical value determining whether the UTM grid reference(s) should be estimated using the centre point of the MGRS grid reference square or the lower/left corner
#' @param return_precision Logical value determining 
#'
#' @return A vector of UTM grid reference(s) or if \code{return_precision = TRUE} a data.frame containing the UTM grid refences in addition to the precision (in metres) of the grid references.
#' @export
#' 
#'@seealso \code{\link{MGRStoUTM}}
#'
#' @examples
#' ## Using the mgrs2utm function
#' 
#' mgrs2utm("30UWV6548")
#' 
#' ## Outside of the UK
