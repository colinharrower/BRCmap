...
gr2sf_poly = function(gridref, gr_atts = NULL, sep_proj_layers = FALSE, crs = NULL){
  ...
  # find any non-valid grid refs
    ...
    if(length(bad_gr) > 0){
      ...
      warning(length(bad_gr)," non-valid grid ref(s) found and excluded")
    }
    gridref = temp
  
  # Determine number of gridrefs
...
#' Convert grid references to spatial simple features points object
#' @description Takes one or more grid references, and any associated data or
#'   attributes, and converts them to a simple features points object.
#'
#' @param gridref A character vector containing the grid references to be
#'   converted.
#' @param gr_atts A data.frame of associated attribute information or data to be
...
#'   object or a list of simple features points objects. The output will
#'   depend on the grid reference type(s) of the grid references supplied to
#'   \code{gridref}, whether a Coordinate Reference system is supplied to
#'   \code{crs}, and or whether \code{sep_proj_layers} is \code{TRUE} or
#'   code{FALSE}
#'
#' @details Each grid reference in \code{gridref} will be converted to a point
#'   in the simple features points object. This means that if
#'   \code{gridref} contains duplicated grid references multiple overlapping
#'   points will be present in the final simple features object. This may be
#'   desirable if the attribute information attached to each instance of the
#'   same grid references is different, but may also be undesirable in many
#'   situations. If you want a single point for each grid reference then you
#'   will need to summarise your grid reference and attribute data first to
#'   ensure there is only a single entry/row for each gird reference.
#'
#'   When \code{sep_project_layers = FALSE} a single simple feature points
#'   object will be returned using either the CRS for the grid reference system
#'   most frequently represented in \code{gridref} or the CRS specified by
#'   \code{crs} when \code{crs} is not \code{NULL}. When
#'   \code{sep_project_layers = TRUE} a list of simple features will be returned
#'   with each element containing a simple feature mulitpoints object containing
#'   all grid references in \code{gridref} coming a single grid ref system (e.g.
#'   OSGB, OSI, UTM30), and using the corresponding crs, with the number of
#'   elements in the list depending on the number of grid references types
#'   present in \code{gridref}.
#'   
#'   These functions to convert to simple feature (sf) objects will ultimately 
#'   replace the old legacy functions that convert grid reference data to 
#'   spatial objects using the \code{sp} package.
...
#' @examples
#'
#' ## Example converting a a series of grid references to a simple features points object
#'  # Create a vector of grid references
#'    gridrefs <- c("SP49","WV57","H40","NN29","NT20","TL97")
#'
#'  # Create a data.frame of associated data
#'    gr_attributes = data.frame(site = paste("site",1:length(gridrefs)), VC = c(55,113,230,97,72,26))
#'
#'  # convert grid refs to simple features object with a point for the center of each grid ref
...
gr2sf_points = function(gridref, gr_atts = NULL, sep_proj_layers = FALSE, crs = NULL){
  ...
  if(length(bad_gr) > 0){
    # if non-valid found then warn
    warning(length(bad_gr)," non-valid grid ref(s) found and excluded")
    # Now exclude empty/bad indices from temp
    temp = temp[-bad_gr]
  }
  gridref = temp
  ...
  for(i in 1:length(gr_tp)){	
    ...
    # Find centres of grid refs
      gr_cent = as.matrix(gr_let2num(gridref[inds], centre = TRUE))
    ...
    # TODO Might be more efficient to convertcoordinates to coordinates in the output projection using sf_project before creating sf object as can sometimes be performance issues with point/multipoint data and st_transform?

      
    # Convert data.frame of coordinates to a list of one row
      pt_lt = lapply(split(gr_cent,1:nrow(gr_cent)),matrix, nrow=1)
    # Create sf point object for gridrefs from current CRS
      sp_pts = lapply(pt_lt,function(x){sf::st_point(x)})
    # Create a sf geometry list column
      sp_sfc = sf::st_sfc(sp_pts, crs = tp_crs)
    # Cleanup
      rm(gr_cent, pt_lt, sp_pts)
    # Now bind with attribute data and create full simple feature object
    # Create data.frame to associate with features
    sp_df = data.frame(GRIDREF = gridref[inds])
    if(!is.null(gr_atts)){
    ...
    row.names(sp_df) = NULL
    sp_sf = sf::st_sf(sp_df,geometry = sp_sfc,row.names = 1:nrow(sp_df))
    rm(sp_df)	
    # Test whether CRS argument is not NULL (i.e. it was supplied or either sep_proj_layers = FALSE or there is only 1 grid ref type present)		
    if(!is.null(crs)){
      # Test whether CRS of current gr_tp matches output CRS if not will need to reproject it
      if(sf::st_crs(sp_sf) != sf::st_crs(crs)){
        sp_sf = sf::st_transform(sp_sf,crs = crs)
        #sp_sf = sf::sf_project(from = sf::st_crs(sp_sf), to = crs, pts = )
      }
    }
